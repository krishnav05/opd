    ...
    <div class="fluid-container">
      <div class="row m-0">
        ...
        <div class="col text-right pr-0"> <a class="credit text-center" href="" onclick="return false;"> <span class="
          credit-box">@if(Auth::check() && Auth::user()->role_id == 3) <img src="/storage/users-avatar/{{Auth::user()->avatar}}" style="border-radius: 50%; height: 40px; width: 40px;" > image @else @if(isset($credit)) {{$credit}} @else 0 @endif @endif</span> <span class="d-block">@if(Auth::check() && Auth::user()->role_id == 3) @else Credits @endif</span> </a> </div>
        </div>
      </div>
    </header>
